...
func encriptarPass(pass string, clave string) string { //PRESENTACION encriptado doble
...
func encontrarEmail(w http.ResponseWriter, r *http.Request) { // NOTE encontrar email
...
func findUsuarioByEmail(mail string) (bool, string) {
	...
	fmt.Println("Valor de mail en findUsuarioByEmail: 345", mail)
	db, err := obtenerBaseDeDatos()
	if err != nil {
		fmt.Printf("Error obteniendo base de datos: %v", err)
	}
...
func verificarCodigo(w http.ResponseWriter, r *http.Request) {
	...
	if err1 != nil {
		fmt.Printf("Error obteniendo base de datos: %v", err1)
		return
	}
	defer db.Close()
	code := &models.Usuario{}
...
func encontrarUsuario(password, email string) models.Value { //NOTE encontrar usuario
	...
	if resultVacio {
		...
		value := models.Value{Id, Nombre, Rol, Token}
		return value
	}
	...
	value := models.Value{id, nombre, rol, tokenString}
...
func crearToken(id, nombre, email string) string { //PRESENTACION -- Crear token
	...
	guardarToken(tokenString, id) // GUARDA EN BD

	return tokenString
}

func verificarToken(tknStr, SecretKey string) int { //PRESENTACION 2 retorna 0 ok, 1 invalid, 2 otros casos(errores)
	resp := 0 // ok
	claims := &models.Claims{
		// Email: SecretKey,
	}
...
func crearCookie(w http.ResponseWriter, r *http.Request, value models.Value) {
	fmt.Println("crearCookie")
...
func verificarCookies(w http.ResponseWriter, r *http.Request) int { // PRESENTACION Verifica cookie
	...
	tknStr := c.Value //NOTE valor del token en la cuki del usuario
	...
	secret := usuario.Email //clave para cada usuario

	resp := verificarToken(tknStr, secret) //NOTE verifica Token

	if resp == 0 { //NOTE VALOR DE VERIFICAR
		token := crearToken(usuario.Id, usuario.Nombre, usuario.Email) // NOTE refresca token
		...
		eliminarCookie(w, r)
		crearCookie(w, r, value) //NOTE nueva cookie
	}
...
/////////////////////// GET TODO ////////////////////
func getAll(w http.ResponseWriter, r *http.Request) { // PRESENTACION Una sola peticion http
	db, err := obtenerBaseDeDatos()
	if err != nil {
		fmt.Printf("Error obteniendo base de datos: %v", err)
		return
...
/////////////////////// POST AUTOR ////////////////////
func postAutor(w http.ResponseWriter, r *http.Request) { //PRESENTACION Refrescar
	w.Header().Set("Content-Type", "application/json")
	db, err := obtenerBaseDeDatos()
	if err != nil {
		fmt.Printf("Error obteniendo base de datos: %v", err)
	...
	resp := verificarCookies(w, r) //NOTE verifica cooki
...
func postUsuario(w http.ResponseWriter, r *http.Request) { //PRESENTACION 0 Post Usuario
	//  fmt.Println("post usuario 1655")
	db, err := obtenerBaseDeDatos()
	if err != nil {
		fmt.Printf("Error obteniendo base de datos: %v", err)
		return
	...
	if err != nil {
		fmt.Println("post usuario error1")
		//	panic(err.Error())
	}

	body, err := ioutil.ReadAll(r.Body)
	if err != nil {
		fmt.Println("post usuario error2")
		//		panic(err.Error())
	}
	key := make(map[string]string)
	json.Unmarshal(body, &key)
	...
	pass := encriptarPass(password, email) //NOTE llama a Encriptar
	tok = crearToken(id, nombre, email)    //NOTE crea JWT
	//	fmt.Println("post usuario vuelve de crear token")
	_, err = stmt.Exec(&id, &nombre, &pass, &email, &rol, &tok, &codigo)
	if err != nil {
		fmt.Println("post usuario error3")
	...
	result, err := db.Query("SELECT * FROM usuarios WHERE email like ?", &email)
	if err != nil {
		fmt.Println("post usuario error4")
	...
	for result.Next() {
		err := result.Scan(&usuario.Id, &usuario.Nombre, &usuario.Password, &usuario.Email, &usuario.Rol, &usuario.Tok, &usuario.Codigo)
		if err != nil {
			fmt.Println("post usuario error5")

			//		panic(err.Error())
		}
	}

	value := models.Value{usuario.Id, usuario.Nombre, usuario.Rol, usuario.Tok}
